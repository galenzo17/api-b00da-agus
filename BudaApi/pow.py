...
class BudaProof():
    ...
    def pown(self):  # implementation with selection between md5, sha256 and sha512
        found = False
        nonce = randint(1000, 100000)
        count = 0
        #text = input("Please type in the text: ")
        text = self.text
        strnulls = 4 #int(input("Type in leading nulls (in decimal! like: 4 = 16 binary nulls): "))
        nulls = self.makeStringNulls(strnulls)
        try:
            mode = 1#int(input("Choose mode: SHA256 = 1, SHA512 = 2, MD5 = 3 (hashing method not safe): "))
        except:
        ...
        while not found:
            if mode == 1:
                ...
                if hasht.startswith(nulls):
                    found = True
                    #return json({"hash": hasht, "text": text, "nonce": nonce, "count": count})
                    self.nonce = nonce
                    print("Hash found! " + str(hasht))
                    print("Text and Nonce: " + text + " + " + str(nonce) + " count: " + str(count) + "\n")
                nonce += 1
    ...
    def makeStringNulls(self,nulls):
        strnulls = ""
    ...
    def start(self):
        ...
        while not starti:
            try:  # catch errors by typing in
                startinput = 1#int(input("Start Proof of Work? 1= Yes, 9= No: "))
                if startinput == 1:
                    self.pown()
                elif startinput == 9:
                    print("Stopped!")
